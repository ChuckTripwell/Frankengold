...
KERNEL_DIR="$(find "${TARGET_DIR}/usr/lib/modules" -mindepth 1 -maxdepth 1 -type d -name "[0-9]*" | sort -V | tail -n 1)"
...
depmod -b "${TARGET_DIR}" -a "$KERNEL_VER"

log "Configuring MOK enrollment service..."
mkdir -p "${TARGET_DIR}/usr/share/cert" "${TARGET_DIR}/usr/lib/systemd/system"
...
echo "DefaultDependencies=no" >> "$SERVICE"
echo "After=local-fs.target" >> "$SERVICE"
echo "Before=sysinit.target" >> "$SERVICE"
echo "" >> "$SERVICE"
echo "[Service]" >> "$SERVICE"
echo "Type=oneshot" >> "$SERVICE"
echo "RemainAfterExit=yes" >> "$SERVICE"
echo "ExecStart=/bin/bash -c 'KERNEL_VERSION=\$(uname -r); STAMP=\"/etc/.mok_enrolled\"; SAVED_VERSION=\$(cat \"\$STAMP\" 2>/dev/null || echo \"\"); if [ \"\$SAVED_VERSION\" != \"\$KERNEL_VERSION\" ]; then if ujust enroll-secure-boot-key; then echo \"\$KERNEL_VERSION\" > \"\$STAMP\"; fi; fi'" >> "$SERVICE"
echo "" >> "$SERVICE"
echo "[Install]" >> "$SERVICE"
echo "WantedBy=sysinit.target" >> "$SERVICE"

chmod 0644 "$SERVICE"

mkdir -p "${TARGET_DIR}/usr/lib/systemd/system/sysinit.target.wants"
ln -sf /usr/lib/systemd/system/mok-enroll.service "${TARGET_DIR}/usr/lib/systemd/system/sysinit.target.wants/mok-enroll.service"
